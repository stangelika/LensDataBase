    ...
    var body: some View {
        VStack(spacing: 0) {
            Button(action: {
                ...
                HStack {
                    Image(systemName: icon).font(.headline.weight(.semibold)).foregroundColor(.white.opacity(0.8))
                        .frame(width: 20)
                    Text(title).font(.headline).foregroundColor(.white.opacity(0.6))
                    Spacer()
                    Text(selection != nil ? selectionDisplay(selection!) : "Выберите...")
                        .font(.headline.weight(.medium)).foregroundColor(.white).lineLimit(1)
                    Image(systemName: "chevron.up.chevron.down").font(.system(size: 14, weight: .bold))
                        .foregroundColor(.white.opacity(0.7)).rotationEffect(.degrees(isExpanded
                                ? 180
                                : 0))
                }
