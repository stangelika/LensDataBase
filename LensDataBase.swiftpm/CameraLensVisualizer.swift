...
struct CameraLensVisualizerRoot: View {
    ...
    var body: some View {
        NavigationView {
                ...
                VStack(spacing: 16) {
                    ...
                    VStack(spacing: 12) {
                        ...
                        pickerId: .camera, title: "Camera", icon: "camera", data: dataManager.cameras, selection: $selectedCamera, activePicker: $activePicker, selectionDisplay: {
                            ...
                            PickerRowView(
                            text: "\(camera.manufacturer) \(camera.model)", isSelected: selectedCamera? .id    == camera.id
                            )
                        }
                        )
                        ...
                        if let camera = selectedCamera {
                            ...
                            pickerId: .format, title: "Format", icon: "aspectratio", data: dataManager.formats.filter {
                                ...
                                format in
                                PickerRowView(
                                text: "\(format.recordingFormat) (\(format.recordingWidth)x\(format.recordingHeight)мм)", isSelected: selectedFormat? .id    == format.id
                                )
                            }
                            )
                        }
                    ...
                    if let format = selectedFormat, let camera = selectedCamera {
                        ...
                        value: {
                            ...
                            let diag = format.recordingDiagonal.map {
                        ...
                        status: {
                            ...
                            return lensCircle >= formatDiag ? (true, "Полное покрытие", "checkmark.circle.fill") : (false, "Возможна виньетка", "xmark.circle.fill")
                        }()
                        )
                        .padding(.horizontal)
            ...
            Color(.sRGB, red: 28 / 255, green: 32 / 255, blue: 48 / 255), Color(.sRGB, red: 38 / 255, green: 36 / 255, blue: 97 / 255), ]), startPoint: .topLeading, endPoint: .bottomTrailing
            )
            .ignoresSafeArea()
            )
            .onAppear {
        ...
        .onChange(of: selectedCamera) {
            ...
            if let cam = newCamera {
                selectedFormat = dataManager.formats.first(where: {
                    $0.cameraId    == cam.id
                }
                )
            }
...
extension Lens {
    var validImageCircle: Double? {
        ...
        guard !cleanedValue.isEmpty, cleanedValue    != " - " else {
            return nil
        }
...
extension RecordingFormat {
    var validRecordingWidth: Double? {
        cleanAndConvertToDouble(recordingWidth)
    }
    var validRecordingHeight: Double? {
        cleanAndConvertToDouble(recordingHeight)
    }
    var recordingDiagonal: Double? {
        guard let width = validRecordingWidth, let height = validRecordingHeight else {
            return nil
        }
