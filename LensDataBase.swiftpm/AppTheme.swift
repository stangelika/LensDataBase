...
enum AppTheme {
    ...
    enum Shadows {
        ...
        static func filterShadow(color: Color, isActive: Bool) -> some View {
            EmptyView().shadow(
                color: color.opacity(isActive ? 0.20: 0.07), radius: isActive ? 9: 3, x: 0, y: 3
            )
        }
    ...
    enum Modifiers {
        static func glassCard(
            cornerRadius: CGFloat = CornerRadius.large, borderColor: Color = Colors.cardBorder, borderWidth: CGFloat = 1
        ) -> some ViewModifier {
            GlassCardModifier(
        ...
        static func materialBackground(
            cornerRadius: CGFloat = CornerRadius.large, material: Material = .ultraThinMaterial
        ) -> some ViewModifier {
        ...
        static func filterChip(
            accentColor: Color, isActive: Bool = false, cornerRadius: CGFloat = CornerRadius.large
        ) -> some ViewModifier {
            FilterChipModifier(
            ...
            .background(
                ZStack {
                    ...
                    RoundedRectangle(cornerRadius: cornerRadius)
                        .stroke(
                            isActive ? accentColor.opacity(0.7): accentColor.opacity(0.28), lineWidth: isActive ? 2.3: 1.3
                        )
                }
            )
            .shadow(
                color: accentColor.opacity(isActive ? 0.20: 0.07), radius: isActive ? 9: 3, x: 0, y: 3
            )
...
extension View {
    func glassCard(
        cornerRadius: CGFloat = AppTheme.CornerRadius.large, borderColor: Color = AppTheme.Colors.cardBorder, borderWidth: CGFloat = 1
    ) -> some View {
    ...
    func materialBackground(
        cornerRadius: CGFloat = AppTheme.CornerRadius.large, material: Material = .ultraThinMaterial
    ) -> some View {
    ...
    func filterChip(
        accentColor: Color, isActive: Bool = false, cornerRadius: CGFloat = AppTheme.CornerRadius.large
    ) -> some View {
