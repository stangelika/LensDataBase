    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            LazyVStack(spacing: AppTheme.Spacing.padding36) {
                ...
                Spacer(minLength: 250) // мягкий отступ чтобы последний элемент не прилипал к краю
            }
            .padding(.top, AppTheme.Spacing.xl)
        }
        // .background(...) убран, фон наследуется от родителя
    }
}
struct WeatherStyleLensSeriesView: View {
    ...
    var body: some View {
        ...
        // УДАЛИ .animation(.easeInOut(duration: 0.18), value: isExpanded)
    }
}
