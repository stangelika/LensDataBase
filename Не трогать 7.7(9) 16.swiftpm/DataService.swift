// DataService.swift (или DataManager.swift) - ПОЛНЫЙ КОД С ИЗМЕНЕНИЯМИ
...
class DataManager: ObservableObject {
    ...
    func addProject(withName name: String) {
    let newProject = Project(name: name) // Предполагается, что у Project есть такой инициализатор
    projects.insert(newProject, at: 0) // Вставляем в начало списка для наглядности
    // saveProjects() будет вызван автоматически через didSet
}
    ...
    // ✅ НАЧАЛО: НОВАЯ ФУНКЦИЯ ДЛЯ УДАЛЕНИЯ ИЗ ДЕТАЛЬНОГО ЭКРАНА
