...
class DataManager: ObservableObject {
    ...
    // MARK: - Фильтры для AllLensesView (новые @Published свойства)
    ...
    // Результат вычислений для отображения
    @Published var groupedAndFilteredLenses: [LensGroup] = []

    // ИЗМЕНЕНО: Убрано filterPipelineCancellable? если мы будем использовать общий Set
    private var cancellables = Set<AnyCancellable>() 
    private let favoritesKey = "favoriteLenses"

    init() {
        loadFavorites()
        setupFilterPipeline() // Настраиваем Combine-цепочку при инициализации
    }
    ...
    func loadData() {
        ...
        Publishers.Zip(lensPublisher, cameraPublisher)
            ...
            .store(in: &cancellables) // Store in the common cancellables set
    }
    ...
    // MARK: - Вспомогательные функции (доступные извне)

    // Метод для настройки Combine-цепочки для фильтрации и группировки линз
    private func setupFilterPipeline() {
        ...
        // ИЗМЕНЕНО: Store the cancellable directly into the 'cancellables' Set
        filterCombiner
            .map { [weak self] appData, format, focalCategory, showOnlyRentable in
            ...
            .store(in: &cancellables) // <-- CORRECTED: Store in the main 'cancellables' set
    }

    // НОВАЯ ФУНКЦИЯ: Инкапсулирует логику фильтрации и группировки
    private func processLensesForGrouping(appData: AppData, format: String, focalCategory: FocalCategory, showOnlyRentable: Bool) -> [LensGroup] {
        let baseLenses: [Lens]
        if showOnlyRentable {
            let allRentableLensIDs = Set(appData.inventory.values.flatMap { $0.map(\.lens_id) })
            baseLenses = appData.lenses.filter { allRentableLensIDs.contains($0.id) }
        } else {
            baseLenses = appData.lenses
        }

        let normalizedManufacturers = baseLenses.reduce(into: [String: [Lens]]()) { result, lens in
            let normalized = self.normalizeName(lens.manufacturer)
            result[normalized, default: []].append(lens)
        }

        let initialGroups = normalizedManufacturers.map { manufacturerKey, lenses in
            ...
            let series = normalizedSeries.map { seriesKey, seriesLenses in
        ...
        guard !format.isEmpty || focalCategory != .all else { return initialGroups }
        
        return initialGroups.compactMap { group in
    ...
    func lensesForRental(_ rentalId: String) -> [Lens] {
        guard let appData, let inventory = appData.inventory[rentalId] else { return [] }
